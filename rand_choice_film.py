...
def choice():
    """
    Запрашивает у пользователя список фильмов и выбирает случайный фильм.
    """
...
def main():
    ...
    while True:
        choice()  # Вызываем функцию выбора фильма
        # Запрос: продолжение или выход
        select = input("\n Хотите выбрать фильмы заново? ДА/НЕТ: ").strip().upper()
        if select == "ДА":
            continue    # Если пользователь хочет продолжить, выходим из этого цикла
        elif select == "НЕТ":
            print("🌈Всего доброго. Ждём вас снова!🌈") 
            return  # Завершаем программу
        else:
            print("❌Ответ введён не корректно. Ответьте ДА/НЕТ❌")
